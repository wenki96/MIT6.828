...
在命令行输入的时候发现backspace有光标只后退不刷新的bug，并且在没有字符时会输出"\x7f(\b)"这个奇怪字符，所以在/lib/readline.c里面改了下
...
2. console.c中的下面这段代码作用是实现屏幕滚动一行。
	...
	7      }
	```
3. fmt指向的是参数中的格式字符串，而ap指向fmt的后一个参数地址，具体需要理解printf中用到的va_list, va_arg(), va_start(), va_end()，实现起来很巧妙，但是带来的震撼没有之前学习variadic template大，可参考http://www.cnblogs.com/justinzhang/archive/2011/09/29/2195969.html。
...
6. 如果GCC参数入栈方式改为从左往右，则表示参数个数的参数需要在末尾传入cprintf函数，而非开始。
...
```
从 `obj/kernel/kernel.asm`中可以看到栈顶是 `0xf0110000`，栈是由高地址向低地址扩展的。
...
mon_backtrace(int argc, char **argv, struct Trapframe *tf)
{
	...
	while(ebp){
		uint32_t eip = *(ebp + 1), *args = ebp + 2;
		
		cprintf("  ebp %x eip %x args", ebp, eip);
		for(int i=0; i<5; i++) cprintf(" %08x", args[i]);
		cprintf("\n");
...
Stack backtrace:
  ebp f010ff18 eip f0100078 args 00000000 00000000 00000000 f010004a f0111308  // mon_backtrace
  ebp f010ff38 eip f01000a1 args 00000000 00000001 f010ff78 f010004a f0111308  // test_backtrace(5)
  ...
  ebp f010fff8 eip f010003e args 00000003 00001003 00002003 00003003 00004003  // i386_init
...
- [http://www.cnblogs.com/justinzhang/archive/2011/09/29/2195969.html](http://www.cnblogs.com/justinzhang/archive/2011/09/29/2195969.html)
