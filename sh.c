...
runcmd(struct cmd *cmd)
{
  ...
  case ' ':
    ...
    if(ecmd->argv[0] == 0)
      _exit(0);
    // Your code here ...
    if(access(ecmd->argv[0], F_OK) == 0) execv(ecmd->argv[0], ecmd->argv); 
    else{
      char path[100];
      strcpy(path, "/bin/");
      strcat(path, ecmd->argv[0]);
      if(access(path, F_OK) == 0) execv(path, ecmd->argv);
      else fprintf(stderr, "%s command not found", ecmd->argv[0]);
    }
    break;
  ...
  case '<':
    ...
    if(open(rcmd->file, rcmd->flags) < 0){
      fprintf(stderr, "open file %s faild\n", rcmd->file);
      _exit(0);
    }
    runcmd(rcmd->cmd);
    break;
